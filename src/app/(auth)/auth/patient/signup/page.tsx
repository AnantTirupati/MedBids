...
import { HeartPulse, Mail, Lock, User, Phone, Eye, EyeOff, UserPlus } from "lucide-react";
...
export default function PatientSignupPage() {
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await userRepository.createProfile(newPatient);

      // Trigger verification email
      try {
        await authService.sendEmailVerification(user);
      } catch (err) {
        console.warn("[AuthSignup] Failed to dispatch initial verification email:", err);
      }
      ...
      setError(errorObj?.message || "Failed to create your account. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card className="auth-card w-full max-w-[480px] p-6 md:p-8 shadow-2xl flex flex-col items-center select-none bg-[#141A24]/90 border border-outline-variant/20 rounded-2xl relative overflow-hidden backdrop-blur-md">
      ...
      <div className="w-full flex flex-col items-center mb-6 text-center">
        ...
        <p className="text-body-sm text-on-surface-variant">Sign up as a Patient Partner</p>
      </div>
      ...
      <p className="text-body-sm text-on-surface-variant mt-6 text-center">
        ...
        <Link href={`/auth/patient/login${redirectPath ? `?redirect=${redirectPath}` : ""}`} className="text-[#FF6B35] font-semibold hover:underline">
          Sign In
        </Link>
      </p>
    </Card>
